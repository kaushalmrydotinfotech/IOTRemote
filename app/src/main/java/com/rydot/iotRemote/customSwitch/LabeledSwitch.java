...
public class LabeledSwitch extends ToggleableView {
    ...
    private void initView() {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
            colorBorder = colorOn = getResources().getColor(R.color.green, getContext().getTheme());
        } else {
            colorBorder = colorOn = getResources().getColor(R.color.green);
        }
    ...
    private void initProperties(AttributeSet attrs) {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            if (attr == R.styleable.Toggle_on) {
                ...
                int accentColor;
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
                    accentColor = getResources().getColor(R.color.green, getContext().getTheme());
                } else {
                    accentColor = getResources().getColor(R.color.green);
                }
...
//      Drawing Switch Labels here
        ...
        if(isOn) {
            ...
            alpha = (alpha < 0 ? 0 : (alpha > 255 ? 255 : alpha));
            int onColor = Color.argb(alpha, Color.red(colorText), Color.green(colorText), Color.blue(colorText));
            paint.setColor(onColor);
            ...
            alpha = (alpha < 0 ? 0 : (alpha > 255 ? 255 : alpha));
            int offColor = Color.argb(alpha, Color.red(colorText), Color.green(colorText), Color.blue(colorText));
            paint.setColor(offColor);
...
//      Drawing Switch Thumb here
        {
            int alpha = (int) (((thumbBounds.centerX() - thumbOffCenterX) / (thumbOnCenterX - thumbOffCenterX)) * 255);
            alpha = (alpha < 0 ? 0 : (alpha > 255 ? 255 : alpha));
            int offColor = Color.argb(alpha, Color.parseColor("#FFFFFFFF"),Color.parseColor("#FFFFFFFF"),Color.parseColor("#FFFFFFFF"));
            paint.setColor(offColor);
